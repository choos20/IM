...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_member'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        $photo = "../uploads/" . basename($_FILES["photo"]["name"]); // relative for browser
    }
...
<head>
  ...
  <link rel="stylesheet" href="../ALL_CSS/members1.css"> <!-- external stylesheet -->
</head>
<body>
  <header>
      ...
      <tr>
        <td>
          <?php if (!empty($row['photo'])): ?>
            <img src="<?= htmlspecialchars($row['photo']) ?>" class="member-photo" alt="">
          <?php else: ?>
            <span>No Photo</span>
          <?php endif; ?>
        </td>
